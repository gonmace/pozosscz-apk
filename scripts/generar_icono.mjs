 ...
 *   node scripts/generar_icono.mjs
 */
...
<svg xmlns="http://www.w3.org/2000/svg"
  ...
  <g transform="translate(${TX}, ${TY + SHADOW_OFFSET_Y}) scale(${TW / 270})"
     filter="url(#blur)" opacity="${SHADOW_OPACITY}">
    <path fill="black" d="${TRUCK_PATH}"/>
  </g>
</svg>`);

// ── 3. SVG del fondo: degradado azul oscuro + esquinas redondeadas ────────
const svgFondo = Buffer.from(`
<svg xmlns="http://www.w3.org/2000/svg"
     width="${SIZE}" height="${SIZE}" viewBox="0 0 ${SIZE} ${SIZE}">
  <defs>
    <linearGradient id="bg" x1="0" y1="0" x2="0.4" y2="1">
      <stop offset="0%"   stop-color="#242840"/>
      <stop offset="100%" stop-color="#0D0F1A"/>
    </linearGradient>
  </defs>
  <rect width="${SIZE}" height="${SIZE}" rx="215" ry="215" fill="url(#bg)"/>
</svg>`);

// ── Composición ───────────────────────────────────────────────────────────
const [bufFondo, bufSombra, bufCamion] = await Promise.all([
  sharp(svgFondo).png().toBuffer(),
  sharp(svgSombra).png().toBuffer(),
  sharp(svgCamion).png().toBuffer(),
]);

const resultado = await sharp(bufFondo)
  .composite([
    { input: bufSombra, blend: 'multiply' },
    { input: bufCamion, blend: 'over' },
  ])
  .png()
  .toBuffer();

writeFileSync('assets/images/icon.png', resultado);
console.log(`Icono generado: assets/images/icon.png (${SIZE}x${SIZE})`);
